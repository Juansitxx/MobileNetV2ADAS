...
| Class imbalance | Not specified | `reports/sampling_plan.md` reports minority/majority ratio and a class_weight recommendation. Real counts showed severe PEDESTRIAN imbalance, which additionally drove a multi-label reformulation (two independent binary targets instead of 4-class softmax) so the minority class is not collapsed by the majority classes during training | `scripts/04_build_experiment_subset.py`, `src/neurodriver_cnn/models/{baseline,mobilenetv2,resnet50_teacher}.py`, `docs/decisions_log.md` (2026-09-22) |
