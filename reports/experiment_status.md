...
| Notebook 01 (baseline + MobileNetV2 + ResNet50 Teacher) | DONE — regenerated 2026-09-22: fixed a `prefetch`-ordering bug (was applied before augmentation instead of last), added explicit loss/val_loss/binary_accuracy/val_binary_accuracy display, and added a MobileNetV2 frozen-backbone training + evaluation section. Not executed end-to-end inside the notebook itself (no real dataset present), but every cell's logic (pipeline, augmentation/prefetch ordering, baseline + MobileNetV2 training and evaluation) has been verified standalone end-to-end with real TensorFlow against a synthetic manifest |
| Baseline CNN (`models/baseline.py`, Student 2) | DONE — final architecture: 4 Conv/Pool blocks (16/32/64/64), `Flatten`, `Dense(64)`, `Dropout`, 2 logits, `BinaryCrossentropy(from_logits=True)`; verified with real TensorFlow: **863,522 total params** (earlier 3-block version was ~12.94M, ~13x over budget — see `docs/decisions_log.md`) |
| MobileNetV2 Student (`models/mobilenetv2.py`, Student 1) | DONE — 2-logit output, `(model, base_model)` return; verified with real TensorFlow: 2,260,546 total params (2,562 trainable, frozen backbone), `(batch,2)` logits, no NaNs; **frozen-backbone training loop added and verified end-to-end (1-epoch run on synthetic data)** |
